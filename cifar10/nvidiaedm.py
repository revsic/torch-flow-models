...
class SongUNet(torch.nn.Module):
    def __init__(
        ...
        self.map_label = (
            Linear(in_features=num_classes + 1, out_features=noise_channels, **init)
            if num_classes >= 2
            else None
        )
        self.map_augment = (
    ...
    def forward(self, x, t, label=None, augment_labels=None):
        # SUPER WEIRD BUT WORK
        with contextlib.redirect_stdout(None):
            print(x.device, next(self.parameters()).device, t.device, label.device)
        # Mapping.
        y = label.to(x.device)
        t = (t.to(x.device) * 2.68 - 1.59).flatten()  # EDM adapted to UCGM
        ...
        if self.map_label is not None:
            ...
            #     ).to(tmp.dtype)
            tmp = torch.eye(self.num_classes + 1, dtype=y.dtype, device=y.device)[tmp]
